(() => {
  ...
  const KICKOFF_KEY = localStorage.getItem(KICKOFF_KEY_STORAGE) || DEFAULT_KICKOFF_KEY;
  const DATA_URL = `${KICKOFF_BASE}/fixtures?league=${KICKOFF_LEAGUE}&season=${KICKOFF_SEASON}`;
  const OPENFOOTBALL_URL = 'https://raw.githubusercontent.com/openfootball/worldcup.json/master/2026/worldcup.json';
  const CACHE_KEY = 'vm2026:data:kickoff:v15';
  ...
  const MATCH_DETAILS_CACHE_KEY = 'vm2026:kickoffMatchDetails:v15';
  const MAX_API_CALLS_PER_MINUTE = 8;
  const MAX_API_CALLS_PER_DAY = 90;
  const MIN_API_INTERVAL_MS = 7500;
  ...
  async function fetchKickoffData(manual = false) {
    if (!KICKOFF_KEY) throw new Error('mangler KickoffAPI nøgle');
  ...
  function formatApiSportsErrors(errors) {
    if (!errors) return '';
  ...
  function shouldFetchFreshData(cached, manual) {
    ...
    const hasLive = matches.some(match => ['1H', 'HT', '2H', 'ET', 'BT', 'P', 'LIVE', 'IN_PLAY', 'PAUSED'].includes(match.apiStatus || match.status));
  ...
  function normalizeKickoffMatch(item) {
    ...
    const goalsObj = (item.goals && !Array.isArray(item.goals) ? item.goals : {}) || fixture.goals || {};
    const score = item.score || {};
    const events = collectKickoffEvents(item);
    const stats = Array.isArray(item.statistics) ? item.statistics : [];
    const home = teams.home || item.homeTeam || item.home || fixture.homeTeam || {};
    const away = teams.away || item.awayTeam || item.away || fixture.awayTeam || {};
    const homeName = teamNameFrom(home, item.home_name || item.homeName || item.home_team || item.home) || 'TBD';
    const awayName = teamNameFrom(away, item.away_name || item.awayName || item.away_team || item.away) || 'TBD';
    const fixtureId = fixture.id || item.id || item.fixtureId || item.fixture_id;
    const rawStatus = fixture.status?.short || fixture.statusShort || item.statusShort || item.status_short || item.status || fixture.status?.long;
    const kickoffDate = fixture.date || item.date || item.utcDate || item.kickoff || item.start_time;
    const fullHome = item.goalsHome ?? item.scoreFullHome ?? goalsObj.home ?? goalsObj.homeTeam ?? home.goals ?? home.score ?? item.home_score ?? item.homeScore ?? item.score?.fullTime?.home ?? item.score?.fulltime?.home ?? item.score?.full_time?.home ?? item.score?.home ?? item.score?.a ?? item.scores?.home ?? item.result?.home;
    const fullAway = item.goalsAway ?? item.scoreFullAway ?? goalsObj.away ?? goalsObj.awayTeam ?? away.goals ?? away.score ?? item.away_score ?? item.awayScore ?? item.score?.fullTime?.away ?? item.score?.fulltime?.away ?? item.score?.full_time?.away ?? item.score?.away ?? item.score?.b ?? item.scores?.away ?? item.result?.away;
    return {
      ...item,
      num: fixtureId,
      sourceType: 'kickoff',
      team1: homeName,
      team2: awayName,
      team1Id: teamIdFrom(home) || item.homeTeamId || item.home_team_id || null,
      team2Id: teamIdFrom(away) || item.awayTeamId || item.away_team_id || null,
      utcDate: kickoffDate,
      ...
      score: {
        ...
        halfTime: { home: item.scoreHalfHome ?? score.halftime?.home ?? score.halfTime?.home ?? item.halftime?.home, away: item.scoreHalfAway ?? score.halftime?.away ?? score.halfTime?.away ?? item.halftime?.away },
        regularTime: { home: item.scoreFullHome ?? score.fulltime?.home ?? score.fullTime?.home, away: item.scoreFullAway ?? score.fulltime?.away ?? score.fullTime?.away },
        extraTime: { home: item.scoreExtraHome ?? score.extratime?.home ?? score.extraTime?.home, away: item.scoreExtraAway ?? score.extratime?.away ?? score.extraTime?.away },
        penalties: { home: item.scorePenaltyHome ?? score.penalty?.home ?? score.penalties?.home, away: item.scorePenaltyAway ?? score.penalty?.away ?? score.penalties?.away }
      },
  ...
  function collectKickoffEvents(item) {
    ...
    if (item.events && Array.isArray(item.events.response)) candidates.push(item.events.response);
    if (item.events && Array.isArray(item.events.data)) candidates.push(item.events.data);
    return candidates.find(Array.isArray) || [];
  }

  function eventKind(event) {
    return String(event?.type || event?.detail || event?.event_type || event?.eventType || event?.kind || '').toLowerCase();
  }

  function isGoalEvent(event) {
    const text = eventKind(event);
    return text.includes('goal') && !text.includes('cancel');
  }
  ...
  function normalizeMatch(match, index) {
    ...
    const status = apiStatus && ['1H', 'HT', '2H', 'ET', 'BT', 'P', 'LIVE', 'IN_PLAY', 'PAUSED'].includes(apiStatus) ? 'live' : score ? 'result' : kickoff && isSameCopenhagenDate(kickoff, now) ? 'today' : 'scheduled';
    return {
      ...match,
      id,
      index,
  ...
  function handleGlobalClick(event) {
    ...
    localStorage.removeItem(CACHE_KEY);
    localStorage.removeItem(MATCH_DETAILS_CACHE_KEY);
    toast(value ? 'KickoffAPI nøgle gemt. Cache ryddet. Genindlæs siden.' : 'API-nøgle fjernet. Cache ryddet.');
  }
  ...
  async function fetchKickoffFullMatchDetails(match) {
    ...
    const hasRecordedScore = Boolean(
      match.score && (
        isNumberLike(match.score.fullTime?.home) ||
        isNumberLike(match.score.fullTime?.away) ||
        isNumberLike(match.score.label?.split('-')?.[0])
      )
    );
    const isFinishedOrLive = ['1H', 'HT', '2H', 'ET', 'BT', 'P', 'FT', 'AET', 'PEN', 'LIVE', 'FINISHED', 'result'].includes(status) || hasRecordedScore;
    const isLikelyUseful = isFinishedOrLive;
    const lineupsMayExist = isLikelyUseful || (minutesToKickoff !== null && minutesToKickoff <= 90);

    const endpointCalls = [
      ['fixtures/events', { fixture: fixtureId }, 'events', isLikelyUseful],
      ['fixtures/statistics', { fixture: fixtureId }, 'statistics', isLikelyUseful],
      ['fixtures/lineups', { fixture: fixtureId }, 'lineups', lineupsMayExist]
    ];

    for (const [endpoint, params, key, shouldCall] of endpointCalls) {
      if (!shouldCall) continue;
      try {
        combined[key] = await kickoffGet(endpoint, params);
      } catch (error) {
        combined.extraErrors = combined.extraErrors || [];
        combined.extraErrors.push(`${endpoint}?fixture=${fixtureId}: ${error.message}`);
      }
    }
  ...
  function renderApiDiagnostics() {
    ...
    const usingDefault = KICKOFF_KEY === DEFAULT_KICKOFF_KEY;
    return `
    ...
    return `
      ...
      <section class="card">
        ...
        <div class="api-key-box">
          ...
          <input id="kickoffKey" type="password" placeholder="KickoffAPI key" value="${escapeAttr(KICKOFF_KEY)}">
